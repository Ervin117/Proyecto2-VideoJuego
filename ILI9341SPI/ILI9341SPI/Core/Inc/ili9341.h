...
void LCD_Sprite_Transparent(int x, int y, int width, int height, const uint16_t *bitmap, int columns, int index, uint16_t transparentColor);
void LCD_RestaurarFondo(int x, int y, int w, int h, const uint16_t *fondo);
void LCD_DibujarSpriteSinParpadeo(int x, int y, int w, int h, const uint16_t *sprite_map, int frame, const uint16_t *fondo_global, uint16_t color_transparente);
